...
const generatePONumber = async () => {
  ...
  const result = await query(`SELECT COUNT(*) FROM purchase_orders WHERE po_number LIKE $1`, [`${prefix}%`]);
...
const getPurchaseRequisitions = async (req, res, next) => {
  try {
    ...
    const result = await query(
      `SELECT pr.*,
              CONCAT('PR/', TO_CHAR(pr.created_at, 'YYYY/MM'), '/', LPAD(pr.id::text, 4, '0')) as pr_number,
              u.full_name as requested_by_name,
              d.name as department_name,
              COALESCE((SELECT SUM(quantity * estimated_unit_price) FROM purchase_requisition_items WHERE pr_id = pr.id), 0) as total_estimated_amount
...
const approvePR = async (req, res, next) => {
  try {
    ...
    await logActivity({
      ...
      description: `PR ${status} oleh ${req.user.username}`,
...
const receiveGoods = async (req, res, next) => {
  try {
    ...
    await transaction(async (client) => {
      ...
      const jeId = je.rows[0].id; const amt = po.rows[0].total_amount;
      ...
      if (persediaan.rows.length && hutang.rows.length) {
        const persediaanId = persediaan.rows[0].id; const hutangId = hutang.rows[0].id;
        await client.query(`INSERT INTO journal_entry_lines (journal_entry_id, account_id, description, debit, credit, line_order) VALUES ($1,$2,$3,$4,0,1)`, [jeId, persediaanId, 'Penerimaan persediaan', amt]);
        await client.query(`INSERT INTO journal_entry_lines (journal_entry_id, account_id, description, debit, credit, line_order) VALUES ($1,$2,$3,0,$4,2)`, [jeId, hutangId, 'Hutang usaha', amt]);
        await client.query(`INSERT INTO general_ledger (account_id, journal_entry_id, entry_date, debit, credit) VALUES ($1,$2,$3,$4,0)`, [persediaanId, jeId, received_date, amt]);
        await client.query(`INSERT INTO general_ledger (account_id, journal_entry_id, entry_date, debit, credit) VALUES ($1,$2,$3,0,$4)`, [hutangId, jeId, received_date, amt]);
      }
...
const approvePO = async (req, res, next) => {
  try {
    ...
    await logActivity({
      ...
      description: `PO ${result.rows[0].po_number} ${status} oleh ${req.user.username}`,
      entityId: id, ipAddress: req.ip, level: 'WARNING'
    });
