  ...
  try {
    ...
    const result = await query(
      `SELECT b.*, p.name as product_name, p.sku,
              (SELECT COUNT(*) FROM bom_items WHERE bom_id = b.id) as material_count
       FROM bom_headers b
...
const createWorkOrder = async (req, res, next) => {
  try {
    ...
    const now = new Date();
    const woNumber = `WO/${now.getFullYear()}/${String(now.getMonth() + 1).padStart(2, '0')}/${Date.now().toString().slice(-4)}`;
...
const completeProduction = async (req, res, next) => {
  try {
    ...
    await transaction(async (client) => {
      ...
      const totalCost = parseFloat(bom.rows[0].material_cost || 0) + parseFloat(labor_cost) + parseFloat(overhead_cost);

      await client.query(
        `INSERT INTO journal_entries (entry_date, reference_number, description, type, total_amount, status, created_by)
         VALUES (NOW(),$1,$2,'EXPENSE',$3,'POSTED',$4)`,
        [`WO-${wo.rows[0].wo_number}`, `Biaya produksi ${wo.rows[0].wo_number}`, totalCost, req.user.id]
      );
